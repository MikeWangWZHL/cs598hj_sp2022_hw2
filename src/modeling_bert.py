...
class BertForSequenceClassificationAddSymbolicFeatures(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
        ...
        self.dropout = nn.Dropout(classifier_dropout)
        self.classifier = nn.Linear(config.hidden_size + config.num_symbolic_features, config.num_labels)
    ...
    def forward(
        ...
        input_ids: Optional[torch.Tensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
        symbolic_features: Optional[torch.Tensor] = None,
        token_type_ids: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.Tensor] = None,
        head_mask: Optional[torch.Tensor] = None,
        inputs_embeds: Optional[torch.Tensor] = None,
        ...
        pooled_output = self.dropout(pooled_output)
        pooled_output = torch.cat((pooled_output, symbolic_features), dim=1)
        logits = self.classifier(pooled_output)
